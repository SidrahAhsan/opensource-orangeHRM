...
export async function loginWithValidCreds() {
  await fill(userName(), 'Admin');
  await fill(password(), 'admin123');
  await click(loginButton());
  await expectPageToHaveURL('https://opensource-demo.orangehrmlive.com/web/index.php/dashboard/index');
}

export async function loginWithInvalidCreds() {
  await fill(userName(), 'invalidUser');
  await fill(password(), 'invalidPassword');
  await click(loginButton());
